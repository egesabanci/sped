...
# Optimal config: checkpoint disabled in _apply_unsloth_lora + per-example cache
python3 << 'PYEOF'
...
# The checkpoint disable is baked into _apply_unsloth_lora
...
# Timed epoch (cache hits + no checkpoint recomputation)
