...
# ── Capture profiling data via Python ──────────────────────────────
python3 << 'PYEOF'
import sys, os, time, torch, gc, json
...
# ── Load models (from 4-bit cache for speed) ───────────────────────
...
# Run 1 training epoch on eval10 (10 examples, bs=1, max_len=4096)
t_start = time.time()
total_tokens = 0
total_steps = 0
...
# Output structured data for the agent to see
print(f"\n=== RESULTS ===")
